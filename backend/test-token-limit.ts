...
async function testWithTokenLimit(limit: number) {
  console.log(`\n=== Testing with max_completion_tokens: ${limit} ===`);
  
  const requestBody = JSON.stringify({
    messages: [
      { role: 'system', content: 'You are a helpful assistant. Always provide a clear answer.' },
...
async function main() {
  await testWithTokenLimit(50);
  await testWithTokenLimit(100);
  await testWithTokenLimit(200);
}
