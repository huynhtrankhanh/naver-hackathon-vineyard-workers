...
import { generateSavingPlan, getGenerationSession } from '../utils/aiService.js';
...
router.get('/stream/:planId', async (req: Request, res: Response) => {
  try {
    ...
    if (!session) {
      ...
      if (plan.streamingStatus === 'completed' || plan.streamingStatus === 'failed') {
        res.write(`data: ${JSON.stringify({
          type: 'complete',
          plan: plan
        })}\n\n`);
        ...
        const pollInterval = setInterval(async () => {
          ...
          if (updatedPlan) {
            ...
            if (updatedPlan.streamingStatus === 'completed' || updatedPlan.streamingStatus === 'failed') {
              res.write(`data: ${JSON.stringify({
                type: 'complete',
                plan: updatedPlan
              })}\n\n`);
